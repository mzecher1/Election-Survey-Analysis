...
#Liam Corrections or suggestions will have three "###"
...
###code repeated from first line so i deleted it
...
###why called June Data Set? If it is data from 2004 election until after?
```
...
```{r}
   ...
   VCF0501 = replace_na(VCF0501, 9),
    ...
    VCF9021 =  replace_na(VCF9021, 3)) -> July_DS_1
        
### you can do what you originally did but you don't need to put the data frame in twice
```

```{r}  
summary("Gender")
### not sure if meant to include but put r code here
```
...
```{r}
###added code for voter access comp
library(readxl)
Voter_Access_Comp_Rankings <- read_excel("GitHub/mz2733/Voter Access-Comp Rankings.xlsx")
View(Voter_Access_Comp_Rankings)
```
...
###Voter_ACcess_Comp_Rankings needs to be imported for reproducibility
```

```{r}
With_State_rank_fil <- subset(Data_State_rank, select = c(YEAR, State, RANKING, `Percent Diff`))
names(With_State_rank)

summary(Voter_Access_Comp_Rankings$Ranking)
###having trouble understanding this block of code. What are you looking for here?
```
...
```{r}
myfile <- "https://github.com/wagner-mspp-2020/mz2733/blob/master/Voter%20Access-Comp%20Rankings.xlsx"

Voter_rank = read.csv (myfile) 
