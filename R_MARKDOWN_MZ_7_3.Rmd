...
TABLE_T_1 <- Data_State_rank %>% janitor::tabyl(Voted_Y_N, Bw_parties)


TABLE_T_2 <-tabyl(Data_State_rank$Voted_Y_N, Data_State_rank$Bw_parties)
...
reg_wsr <- glm(Voted_Y_N_DV ~ Age_Dummy  + Gender_DV + Race_DV + Income_DV+ Education_DV + bw_parties_DV + Party_aff_DV  +  Control_split_DV + Election_Close_DV + Anyone_talk_vote_DV  + Contacted_by_party_DV + + Contacted_Dem_Party_DV + Contacted_Rep_Party_DV + Better_worse_DV + Country_direction_DV    + Dem_Pres_Angry_DV  + Dem_Pres_hopeful_DV + Dem_Pres_Proud_DV + Dem_Pres_Afraid_DV + Rep_Pres_Angry_DV + Rep_Pres_hopeful_DV  +  Rep_Pres_Proud_DV+ Rep_Pres_Afraid_DV   + Interest_elec_DV  + Volunteer_work_DV +Party_aff_DV + RANKING + PercentDiff, weights= Weights, family= binomial, data=Data_State_rank)
...
logit2prob(coef(reg_wsr)) <- function(coef1){
  odds <- exp(coef1)
  prob <- odds / (1 + odds)
  return(prob)
}

skim(Data_State_rank)
summary(Data_State_rank$PercentDiff)
``` 

#Barplot
```{r}
colors <- c("Dark Blue", "Yellow", "Red")

barplot(table(Data_State_rank$Voted_Y_N, Data_State_rank$Bw_parties), main = "Do You Believe There is A Difference Between Dems and Rep?", beside=TRUE, las=1, col = colors, legend=c("Did not vote","Voted"), name=c("No Difference", "Difference", "Don't Know"), ylab= "Did you vote?", xlab= "Is there a difference between major parties?",  weight=Data_State_rank$Weights)
...
barplot(table(Data_State_rank$Voted_Y_N,Data_State_rank$Party_aff,Data_State_rank$Bw_parties), main = "Party Affiliation", beside=TRUE, las=1, col = colors, legend=c("Did not vote","Voted"), name=c("1","2", "3", "4", "5", "6", "7"), ylab= "Did you vote?", xlab= "Is there a difference between major parties?",  weight=Data_State_rank$Weights)
...
table(Data_State_rank$Voted_Y_N,Data_State_rank$Party,Data_State_rank$Bw_parties)
table(Data_State_rank$Voted_Y_N, Data_State_rank$Bw_parties)

        
B <- table(Data_State_rank$Voted_Y_N, Data_State_rank$Bw_parties)

C <- A + B

Data_State_rank  %>%  ggplot(aes(x=Voted_Y_N, fill=Bw_parties)) + geom_bar(color="black", position= "dodge") 

scatterplot(Data_State_rank$Voted_Y_N, Data_State_rank$PercentDiff)
```
plot(Data_State_rank$Voted_Y_N_DV)
abline(Data_State_rank$Voted_Y_N_DV ~ Data_State_rank$Age_dummy,  col="blue", lwd=3)

qplot <- qplot(0,1, xlab="IND DUMMY",ylab="Voting")

plotxscale <- scale_x_continuous(limits=c(0,1))
plotyscale <- scale_y_continuous(limits=c(0,1))
Line1 <- geom_abline(slope=(.5), intercept = (.5), colour= "red", label ="Dummy1", colour= "red")

Line2 <- geom_abline(slope=(.5), intercept = (.3), colour= "red")

arrow <- (arrows(0, .25, 0, .5, length = 0.25, angle = 30,
       code = 2))

Dum_line <-  qplot + Line1 + Line2 + plotxscale + plotyscale
print(Dum_line)
...
```
Data_svy <- as_survey_design(Data_State_rank, weights = Weights)
augment(reg_wsr, data = Data_State_rank) %>% 
